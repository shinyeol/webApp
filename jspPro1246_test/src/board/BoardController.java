...
public class BoardController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(url.indexOf("list.do") != -1) {
			...
			String b_type = request.getParameter("b_type");
			System.out.println("delete 게시판 영역확인 : "+b_type);
			...
			if(comment_count!=0) {
				List<BoardCommentDTO> num_list = bcDao.getComment_num(num);
				for(int i=0;i<num_list.size();i++) {
					bcDao.deleteAll(num_list.get(i));
				}
				bcDao.delete(num);
			}
			...
			request.setAttribute("b_type", b_type);
			System.out.println("view 접근 : "+b_type);
			...
			String b_type = request.getParameter("b_type");
			System.out.println("passwd_check 게시판 영역확인 : "+b_type);
			...
			if(result) {
				...
				page = "/board_servlet/view.do?num="+num+"&message=error&b_type="+b_type;
				response.sendRedirect(contextPath+page);
			}
			...
			int num = Integer.parseInt(request.getParameter("num"));
			String b_type = request.getParameter("b_type");
			System.out.println("reply 접근 : "+b_type);
			...
			String b_type = request.getParameter("b_type");
			System.out.println("insertReply 접근 : "+b_type);
		...
		// myBoard list
		else if(url.indexOf("myBoard.do") != -1) {
			String writer = request.getParameter("writer");
			int count = dao.myBoardCount(writer);
			
			System.out.println("writer : "+writer);
			System.out.println("count : "+count);
			
			List<BoardDTO> list = dao.myBoardList(writer);
			request.setAttribute("list", list);
			request.setAttribute("count", count);

			String page = "/myPage/myBoard.jsp";
			RequestDispatcher rd = request.getRequestDispatcher(page);
			rd.forward(request, response);
		}
